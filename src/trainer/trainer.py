...
# from main_funcs.calculate_all_attentions import calculate_all_attentions
from schedulers.abs_scheduler import (
    AbsBatchStepScheduler,
    AbsEpochStepScheduler,
    AbsScheduler,
    AbsValEpochStepScheduler,
)
from utils.add_gradient_noise import add_gradient_noise
...
from trainer.distributed_utils import DistributedOption
from trainer.reporter import Reporter, SubReporter
from utils.build_dataclass import build_dataclass
from utils.kwargs2args import kwargs2args
...
class Trainer:
    ...
    def run(
        ...
        elif trainer_options.ngpu > 1:
            dp_model = torch.nn.parallel.DataParallel(
                model,
                device_ids=list(range(trainer_options.ngpu)),
            )
        else:
