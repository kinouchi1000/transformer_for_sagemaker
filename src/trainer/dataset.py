...
from utils.sound_scp import SoundScpReader
from utils.sized_dict import SizedDict
from utils.read_text import load_num_sequence_text, read_2column_text
...
def sound_loader(path, float_dtype=None):
    ...
    # TODO(kinouchi): make to be able to handle dir_path
    loader = SoundScpReader(
        path, normalize=True, always_2d=False, dir_path="./test_utils/"
    )
...
class SequenceDataset(AbsDataset):
    ...
    def _build_loader(
        ...
        Args:
            path:  The file path
            loader_type:  loader_type., text_int, text_float, etc
        """
    ...
    def __len__(self):
        ...
        assert len(set(len_list)) == 1, "data length of each loader is not same!!"
        return len_list[0]
